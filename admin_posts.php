...
// busca (força tipo “post” quando nulo/antigo)
$sql = "SELECT id, slug, title, thumb_url, excerpt, published, 
...
<?php else: ?>
    ...
    <?php foreach ($rows as $r): ?>
      <div class="col-12 col-md-6 col-lg-4">
        <div class="card h-100">
          <?php if ($r['thumb_url']): ?>
            <img src="<?= h($r['thumb_url']) ?>" class="card-img-top" style="aspect-ratio:16/9; object-fit:cover">
          <?php endif; ?>
          <div class="card-body d-flex flex-column">
            ...
            <h2 class="h6 mt-1 mb-2"><?= h($r['title']) ?></h2>
            <div class="text-muted small mb-3" style="display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden;">
              <?= h($r['excerpt'] ?? '') ?>
            </div>
            ...
            <div class="mt-auto d-flex justify-content-between align-items-center">
              ...
              <div class="btn-group btn-group-sm">
                <a class="btn btn-outline-primary" href="<?= h(BASE_URL) ?>admin_post_edit.php?id=<?= (int)$r['id'] ?>">Editar</a>

                <?php if ($r['type']==='gallery'): ?>
                  <a class="btn btn-outline-dark" target="_blank" href="<?= h(BASE_URL) ?>galeria-view.php?slug=<?= h($r['slug']) ?>">Ver</a>
                <?php else: ?>
                  <a class="btn btn-outline-dark" target="_blank" href="<?= h(BASE_URL) ?>post.php?slug=<?= h($r['slug']) ?>">Ver</a>
                <?php endif; ?>
  ...
  <?php if ($pages>1): ?>
    <nav class="mt-3">
      <ul class="pagination">
        <?php
          $base = 'admin_posts.php?'.http_build_query(array_filter(['q'=>$q,'published'=>$pub]));
        ?>
